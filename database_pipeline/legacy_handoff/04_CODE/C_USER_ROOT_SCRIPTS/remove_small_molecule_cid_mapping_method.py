...
REMOVE_COLUMN = "cid_mapping_method"
...
def main():
    with SMALL.open(newline="", encoding="utf-8-sig") as f:
        reader = csv.DictReader(f, delimiter="\t")
        original_fields = list(reader.fieldnames or [])
        rows = list(reader)

    if REMOVE_COLUMN not in original_fields:
        raise SystemExit(f"Column not found in {SMALL}: {REMOVE_COLUMN}")
    ...
    update_manifest([SMALL])
    print(json.dumps({
        "small_molecule_rows": len(rows),
