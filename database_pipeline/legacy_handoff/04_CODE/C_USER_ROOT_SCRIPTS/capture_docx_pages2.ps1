param(
  ...
  [string]$OutputDir = 'C:\Users\Administrator\synbio_defense_build\docx-pages'
)
...
public static class WindowCapture2 {
...
try {
  ...
  for ($i = 1; $i -le $pages; $i++) {
    ...
    $rect = New-Object WindowCapture2+RECT
    [WindowCapture2]::GetWindowRect($hwnd, [ref]$rect) | Out-Null
    $width = $rect.Right - $rect.Left
    $height = $rect.Bottom - $rect.Top
    $bitmap = New-Object System.Drawing.Bitmap($width, $height)
    $graphics = [System.Drawing.Graphics]::FromImage($bitmap)
    $hdc = $graphics.GetHdc()
    [WindowCapture2]::PrintWindow($hwnd, $hdc, 2) | Out-Null
