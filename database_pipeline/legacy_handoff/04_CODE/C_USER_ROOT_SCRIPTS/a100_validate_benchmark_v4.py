    ...
    with path.open(encoding="utf-8", errors="ignore") as handle:
        for line in handle:
            ...
            if line.startswith(("ATOM  ", "HETATM")):
                if not in_first and "MODEL" in path.read_text(encoding="utf-8", errors="ignore"):
                    continue
                try:
...
def main() -> int:
    ...
    with OUT.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(rows[0]) if rows else ["task_id"], delimiter="\t")
        writer.writeheader()
        writer.writerows(rows)
    valid = [row for row in rows if row["valid_pair"]]
    score_deltas = sorted(row["abs_score_delta"] for row in valid if row["abs_score_delta"] is not None)
    rmsds = sorted(row["top_pose_direct_rmsd"] for row in valid if row["top_pose_direct_rmsd"] is not None)
    ...
    stable = [row for row in valid if row["abs_score_delta"] <= 1.0 and row["top_pose_direct_rmsd"] is not None and row["top_pose_direct_rmsd"] <= 3.0]
