...
grep -h '^receptor[[:space:]]*=' vina_configs/*.conf \
  | cut -d= -f2- | sed 's/^[[:space:]]*//' | sort -u > "$required"

printf 'receptor\tstatus\n' > logs/receptor_recovery_20260817.tsv
while IFS= read -r rel; do
  [ -n "$rel" ] || continue
  pdb_id="$(basename "$rel" .pdbqt)"
  src="pdb_structures/${pdb_id}.pdb"
  out="receptors_pdbqt/${pdb_id}.pdbqt"
  tmp="${out}.tmp"
  if [ ! -s "$src" ]; then
    printf '%s\tMISSING_PDB\n' "$pdb_id" >> logs/receptor_recovery_20260817.tsv
    continue
  fi
...
# Keep only configurations whose task remains PASS and whose prepared receptor
# and ligand both exist. Never silently substitute another chain or structure.
python3 - <<'PY'
...
for cfg in sorted((root/'vina_configs').glob('*.conf')):
    text=cfg.read_text(encoding='ascii',errors='ignore')
    rec=re.search(r'^receptor\s*=\s*(.+)$',text,re.M)
    lig=re.search(r'^ligand\s*=\s*(.+)$',text,re.M)
    ok=(qc.get(cfg.stem)=='PASS' and rec and lig
        and (root/'receptors_pdbqt_clean'/Path(rec.group(1).strip()).name).is_file()
        and (root/'ligands_pdbqt'/Path(lig.group(1).strip()).name).is_file())
    if ok:
        kept+=1
    else:
        shutil.move(str(cfg),str(blocked/cfg.name)); moved+=1
(root/'metadata/recovery_config_filter_20260817.txt').write_text(
